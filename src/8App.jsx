...
export default function App() {
  ...
  const handleUpdateProduct = (e) => {
    ...
    if (editingProduct.status?.publicada && !isAdmin) {
      setProducts(prev => prev.filter(p => p.id !== editingProduct.id));
      setPendingProducts(prev => [updatedProduct, ...prev]);
    } else if (isAdmin) {
      if (editingProduct.status?.publicada) {
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar py-2 relative">
            ...
            <div className="relative" onClick={(e) => e.stopPropagation()}>
              <button 
                ...
                className={`p-2 rounded-full border transition-all ${selectedConditionGroup === 'outlet' || selectedConditionGroup === 'Ropa nueva con etiqueta' || selectedConditionGroup === 'Ropa nueva sin etiqueta' ? 'bg-red-600 border-red-600' : 'bg-white border-slate-100 hover:border-red-300'}`}
              >
            ...
            <div className="relative" onClick={(e) => e.stopPropagation()}>
              <button 
                ...
                className={`p-2 rounded-full border transition-all ${selectedConditionGroup === 'showroom' || selectedConditionGroup === 'Ropa como nueva' || selectedConditionGroup === 'Ropa usada' ? 'bg-black border-black' : 'bg-white border-slate-100 hover:border-slate-300'}`}
              >
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-6">
                    ...
                    <div className="bg-slate-50 p-3 rounded-xl">
                      ...
                      <p className="font-bold">{selectedProduct.size}</p>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-xl">
          ...
          <div className="relative bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col md:flex-row max-h-[90vh]">
            ...
            <div className="flex-1 p-8 md:p-12 overflow-y-auto no-scrollbar">
              ...
              <form onSubmit={handleUpload} className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <select className="bg-slate-50 border rounded-2xl p-3.5 outline-none" value={newProduct.size} onChange={(e) => setNewProduct({...newProduct, size: e.target.value})}>
                    {SIZES.map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                  <select className="bg-slate-50 border rounded-2xl p-3.5 outline-none" value={newProduct.category} onChange={(e) => setNewProduct({...newProduct, category: e.target.value})}>
                    {CATEGORIES.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                  </select>
                </div>
                
                <div className="grid grid-cols-2 gap-4">
                  <select className="bg-slate-50 border rounded-2xl p-3.5 outline-none" value={newProduct.condition} onChange={(e) => setNewProduct({...newProduct, condition: e.target.value})}>
                    {CONDITIONS.map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                  <select className="bg-slate-50 border rounded-2xl p-3.5 outline-none" value={newProduct.gender} onChange={(e) => setNewProduct({...newProduct, gender: e.target.value})}>
                    {GENDERS.map(g => <option key={g} value={g}>{g}</option>)}
                  </select>
                </div>
                
                <select className="w-full bg-slate-50 border rounded-2xl p-3.5 outline-none" value={newProduct.ageGroup} onChange={(e) => setNewProduct({...newProduct, ageGroup: e.target.value})}>
                  {AGE_GROUPS.map(a => <option key={a} value={a}>{a}</option>)}
                </select>
