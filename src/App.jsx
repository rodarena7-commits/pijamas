...
// Lista inicial de usuarios autorizados para publicar (agregado el nuevo email)
const INITIAL_AUTHORIZED_SELLERS = [
  "rodrigo.n.arena@hotmail.com",
  "ingridhuck_2006@hotmail.com"   // NUEVO: usuario autorizado para publicar
];
...
const [saleConfig, setSaleConfig] = useState(() => {
  ...
  const getDiscountPercentForProduct = (product) => {
    ...
    // Si hay descuento específico para la categoría, usarlo
    if (categoryDiscounts[product.category]) {
      return categoryDiscounts[product.category];
    }
    // Si no, usar descuento global (promo debe ser número)
  ...
  const handleMarkAsSold = async (productId, buyerId, paymentMethod = 'mercadopago', finalPrice = null) => {
    if (window.confirm("¿Marcar esta prenda como vendida?")) {
      ...
      await updateDoc(productRef, {
        ...
        price: priceToUse,   // Actualizamos el precio al de venta con descuento
        montoNeto: montoNeto,
        comisionMonto: comisionMonto
      });
  ...
  const handlePayment = () => {
    ...
    for (let i = 0; i < cart.length; i++) {
      ...
      const { discounted } = getDiscountedPrice(item);
      productosTexto = productosTexto + "- " + item.title + " (Talle: " + talle + ") - $" + discounted;
      if (i < cart.length - 1) {
  ...
  {saleConfig.textoAdicional && ` · ${saleConfig.textoAdicional}`}
      ...
      {/* --- Modal de Perfil de Usuario (sin cambios relevantes para descuento) --- */}
      {selectedUserProfile && (
...
<div className="space-y-2">
  ...
  />
      ...
      {/* --- Modal de Edición de Producto (con descuento) --- */}
