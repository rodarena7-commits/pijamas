...
const INITIAL_PRODUCTS = [
  ...
  { id: 2, user: { name: "Nico_Boutique", avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=Nico" }, image: "https://images.unsplash.com/photo-1562157873-818bc0726f68?auto=format&fit=crop&q=80&w=600", title: "Remera Algodón Pima", price: 18000, size: "L", category: "remeras", condition: "Ropa como nueva", gender: "Masculino", ageGroup: "Juvenil" },
  { id: 3, user: { name: "Street_King", avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=King" }, image: "https://images.unsplash.com/photo-1588850561407-ed78c282e89b?auto=format&fit=crop&q=80&w=600", title: "Gorra Snapback Black", price: 15000, size: "Único", category: "gorras", condition: "Ropa nueva con etiqueta", gender: "Unisex", ageGroup: "Juvenil" }
];
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#FDFCFB] font-sans text-slate-900 pb-20 selection:bg-[#d4af37]/30">
      ...
      {/* --- Otros Modales Existentes (Vender, Carrito) --- */}
      {/* (Se mantiene el código del modal de venta que estaba en el original) */}
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col md:flex-row max-h-[90vh]">
            ...
            <div className="flex-1 p-8 md:p-12 overflow-y-auto no-scrollbar">
              ...
              <form onSubmit={handleUpload} className="space-y-6">
                <input type="text" placeholder="Título" className="w-full bg-slate-50 border rounded-2xl py-3.5 px-5 outline-none" onChange={(e) => setNewProduct({...newProduct, title: e.target.value})} required />
                <input type="number" placeholder="Precio ($)" className="w-full bg-slate-50 border rounded-2xl py-3.5 px-5 outline-none" onChange={(e) => setNewProduct({...newProduct, price: e.target.value})} required />
                ...
                <input type="url" placeholder="URL de la imagen" className="w-full bg-slate-50 border rounded-2xl py-3.5 px-5 outline-none" onChange={(e) => setNewProduct({...newProduct, image: e.target.value})} />
                <button type="submit" className="w-full bg-black text-white py-5 rounded-[2rem] font-bold uppercase text-[10px] tracking-widest mt-4">Publicar Ahora</button>
              </form>
            </div>
          </div>
        </div>
