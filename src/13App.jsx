...
export default function App() {
  ...
  useEffect(() => {
    ...
    const handleBeforeInstallPrompt = (e) => {
      ...
      // Mostrar el prompt personalizado después de 3 segundos
      setTimeout(() => {
    ...
    const showInstallPrompt = (deferredPrompt) => {
      ...
      document.getElementById('install-cancel').addEventListener('click', () => {
        modal.remove();
        // Guardar en localStorage que no quiere instalar
  ...
  const handlePayment = () => {
    ...
    // Redirigir a MercadoPago con el total - SIN TEMPLATE STRINGS
    window.open("https://link.mercadopago.com.ar/masroma?amount=" + total, "_blank");
    
    // Construir mensaje de WhatsApp con los productos - SIN TEMPLATE STRINGS
    let productosTexto = "";
    ...
    // Redirigir a WhatsApp - SIN TEMPLATE STRINGS
              ...
              {showOutletFilters && (
                <>
                  <button
                    ...
                    className={"px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-widest border transition-all whitespace-nowrap " + (selectedOutletFilter === "Ropa nueva con etiqueta" ? 'bg-green-600 border-green-600 text-white shadow-md' : 'bg-white border-slate-100 text-slate-400 hover:border-green-300')}
                  >
                    Nueva con etiqueta
                  </button>
                  <button
                    onClick={() => handleOutletFilterSelect("Ropa nueva sin etiqueta")}
                    className={"px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-widest border transition-all whitespace-nowrap " + (selectedOutletFilter === "Ropa nueva sin etiqueta" ? 'bg-yellow-600 border-yellow-600 text-white shadow-md' : 'bg-white border-slate-100 text-slate-400 hover:border-yellow-300')}
                  >
                    Nueva sin etiqueta
                  </button>
                </>
              ...
              {showShowroomFilters && (
                <>
                  <button
                    ...
                    className={"px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-widest border transition-all whitespace-nowrap " + (selectedShowroomFilter === "Ropa como nueva" ? 'bg-blue-600 border-blue-600 text-white shadow-md' : 'bg-white border-slate-100 text-slate-400 hover:border-blue-300')}
                  >
                    Usado como nuevo
                  </button>
                  <button
                    onClick={() => handleShowroomFilterSelect("Ropa usada")}
                    className={"px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-widest border transition-all whitespace-nowrap " + (selectedShowroomFilter === "Ropa usada" ? 'bg-purple-600 border-purple-600 text-white shadow-md' : 'bg-white border-slate-100 text-slate-400 hover:border-purple-300')}
                  >
                    Usado
                  </button>
                </>
                    ...
                    <div className="flex gap-2 mt-4 overflow-x-auto pb-2">
                      {(selectedProduct.images || [selectedProduct.image]).map((img, idx) => (
                        <button
                          ...
                          className={"flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden border-2 transition-all " + (currentImageIndex === idx ? 'border-[#d4af37] scale-105' : 'border-transparent opacity-60 hover:opacity-100')}
                        >
            ...
            <div className="p-6">
              ...
              <div className="mt-8">
                ...
                {user && user.uid !== selectedUserProfile && (
                  <button
                    ...
                    className={"mt-4 px-6 py-2 rounded-full text-xs font-bold uppercase tracking-widest flex items-center gap-2 transition-all " + (isFollowing(selectedUserProfile) ? 'bg-slate-100 text-slate-600 hover:bg-slate-200' : 'bg-[#d4af37] text-white hover:bg-black')}
                  >
              ...
              <div className="flex gap-4 mt-8 border-b overflow-x-auto no-scrollbar">
                <button
                  ...
                  className={"px-4 py-2 text-sm font-bold uppercase tracking-widest whitespace-nowrap transition-all " + (selectedTab === 'publicaciones' ? 'text-black border-b-2 border-black' : 'text-slate-400 hover:text-slate-600')}
                >
                  Publicaciones ({getUserProducts(selectedUserProfile).filter(p => !p.sold).length})
                </button>
                <button
                  onClick={() => setSelectedTab('vendidos')}
                  className={"px-4 py-2 text-sm font-bold uppercase tracking-widest whitespace-nowrap transition-all " + (selectedTab === 'vendidos' ? 'text-black border-b-2 border-black' : 'text-slate-400 hover:text-slate-600')}
                >
                  Vendidos ({getUserProducts(selectedUserProfile).filter(p => p.sold).length})
                </button>
                <button
                  onClick={() => setSelectedTab('preguntas')}
                  className={"px-4 py-2 text-sm font-bold uppercase tracking-widest whitespace-nowrap transition-all " + (selectedTab === 'preguntas' ? 'text-black border-b-2 border-black' : 'text-slate-400 hover:text-slate-600')}
                >
                  Preguntas ({getUserQuestions(selectedUserProfile).filter(q => !q.answer).length})
                </button>
                <button
                  onClick={() => setSelectedTab('seguidores')}
                  className={"px-4 py-2 text-sm font-bold uppercase tracking-widest whitespace-nowrap transition-all " + (selectedTab === 'seguidores' ? 'text-black border-b-2 border-black' : 'text-slate-400 hover:text-slate-600')}
                >
                  Seguidores ({getFollowersCount(selectedUserProfile)})
                </button>
                <button
                  onClick={() => setSelectedTab('siguiendo')}
                  className={"px-4 py-2 text-sm font-bold uppercase tracking-widest whitespace-nowrap transition-all " + (selectedTab === 'siguiendo' ? 'text-black border-b-2 border-black' : 'text-slate-400 hover:text-slate-600')}
                >
                  Siguiendo ({getFollowingCount(selectedUserProfile)})
                </button>
                <button
                  onClick={() => setSelectedTab('likes')}
                  className={"px-4 py-2 text-sm font-bold uppercase tracking-widest whitespace-nowrap transition-all " + (selectedTab === 'likes' ? 'text-black border-b-2 border-black' : 'text-slate-400 hover:text-slate-600')}
                >
          ...
          <div className="relative bg-white w-full max-w-4xl rounded-[2.5rem] p-8 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex gap-4 mb-6 border-b">
              <button
                ...
                className={"px-4 py-2 text-sm font-bold uppercase tracking-widest " + (selectedTab === 'pendientes' ? 'text-black border-b-2 border-black' : 'text-slate-400')}
              >
                Pendientes ({pendingProducts.length})
              </button>
              <button
                onClick={() => setSelectedTab('publicadas')}
                className={"px-4 py-2 text-sm font-bold uppercase tracking-widest " + (selectedTab === 'publicadas' ? 'text-black border-b-2 border-black' : 'text-slate-400')}
              >
                Publicadas ({products.length})
              </button>
              <button
                onClick={() => setSelectedTab('vendidas')}
                className={"px-4 py-2 text-sm font-bold uppercase tracking-widest " + (selectedTab === 'vendidas' ? 'text-black border-b-2 border-black' : 'text-slate-400')}
              >
                            ...
                            {selectedTab === 'pendientes' && (
                              <>
                                <button 
                                  ...
                                  className={"p-3 rounded-xl transition-all transform hover:scale-105 " + (product.status.verificada ? 'bg-green-100 text-green-600 shadow-md' : 'bg-slate-100 text-slate-400 hover:bg-green-50 hover:text-green-500')}
                                  title="Verificar"
                                >
                                  <CheckCircle size={22} />
                                </button>
                                <button 
                                  ...
                                  className={"p-3 rounded-xl transition-all transform hover:scale-105 " + (product.status.aprobada ? 'bg-green-100 text-green-600 shadow-md' : 'bg-slate-100 text-slate-400 hover:bg-green-50 hover:text-green-500')}
                                  title="Aprobar"
                                >
                                  <Check size={22} />
                                </button>
                          ...
                          <div className="mt-4 bg-slate-100 p-4 rounded-2xl">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-1 flex-1">
                                <div className={"w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold " + (product.status.iniciada ? 'bg-green-500 text-white shadow-lg' : 'bg-white text-slate-400 border-2 border-slate-300')}>
                                  {product.status.iniciada ? <Check size={16} /> : '1'}
                                </div>
                                <div className={"flex-1 h-1 " + (product.status.verificada ? 'bg-green-500' : 'bg-slate-300')}></div>
                                
                                <div className={"w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold " + (product.status.verificada ? 'bg-green-500 text-white shadow-lg' : 'bg-white text-slate-400 border-2 border-slate-300')}>
                                  {product.status.verificada ? <Check size={16} /> : '2'}
                                </div>
                                <div className={"flex-1 h-1 " + (product.status.aprobada ? 'bg-green-500' : 'bg-slate-300')}></div>
                                
                                <div className={"w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold " + (product.status.aprobada ? 'bg-green-500 text-white shadow-lg' : 'bg-white text-slate-400 border-2 border-slate-300')}>
                                  {product.status.aprobada ? <Check size={16} /> : '3'}
                                </div>
                                <div className={"flex-1 h-1 " + (product.status.publicada ? 'bg-green-500' : 'bg-slate-300')}></div>
                                
                                <div className={"w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold " + (product.status.publicada ? 'bg-green-500 text-white shadow-lg' : 'bg-white text-slate-400 border-2 border-slate-300')}>
